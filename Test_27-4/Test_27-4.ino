...
{ 
  ...
  void flam(){
   ...
   if (analogRead(sensorPin0)<analogRead(sensorPin1) && analogRead(sensorPin0)<analogRead(sensorPin2) && analogRead(sensorPin0)<800 ) 
{
  left ();
...
else if (analogRead(sensorPin1)<analogRead(sensorPin0) &&analogRead(sensorPin1)<analogRead(sensorPin2) && analogRead(sensorPin1)<800) 
{
  forward ();
  digitalWrite(BZ,HIGH);
...
else if (analogRead(sensorPin2)<analogRead(sensorPin1) &&sensorValue2<analogRead(sensorPin0) && analogRead(sensorPin2)<800) 
{
  right ();
  digitalWrite(BZ,HIGH);
...
if ( analogRead(sensorPin2)<800 || analogRead(sensorPin1)<800 || analogRead(sensorPin0)<800 ){
  ///send email
...
/// Turn of Power
  ...
  else if ( analogRead(sensorPin1)>40 )
  {
